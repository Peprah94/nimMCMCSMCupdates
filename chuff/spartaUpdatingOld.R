    ...
    for(j in 1:length(names)){
      if(names[j] == latent & length(size[[1]]) > 1){
        ...
        if(length(extraVars) > 0){
          extraValues <- rep(0, length(extraVars))
          names(extraValues) <- extraVars
          allVals <- c(mcmcOut[iter, reducedModel$expandNodeNames(names[j])], extraValues)
        ...
        extraVars <- model$expandNodeNames(names[j])[!model$expandNodeNames(names[j]) %in% reducedModel$expandNodeNames(names[j])]
        if(length(extraVars) > 0){
          estValues <- c(mcmcOut[iter, reducedModel$expandNodeNames(names[j])], rep(0, length(extraVars)))
          names(estValues)[(length(reducedModel$expandNodeNames(names[j])) +1) : (length(reducedModel$expandNodeNames(names[j])) +length(extraVars))] <- extraVars
        }else{
          estValues <- c(mcmcOut[iter, reducedModel$expandNodeNames(names[j])])
        }
