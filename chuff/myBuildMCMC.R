...
my_buildMCMC <- nimbleFunction(
  ...
  setup = function(conf, print, ...) {
    ...
    monitors  <- nimble::mcmc_processMonitorNames(model, conf$monitors)
    monitors2 <- nimble::mcmc_processMonitorNames(model, conf$monitors2)
    ...
    timesConf = modelValuesConf(vars = c('samplerTimes'),
                                type = c( 'double'),
                                size = list(samplerTimes =c(0,0)) )
  ...
  run = function(
    ...
    if(time) customSamplerTimes <- modelValues(timesConf, m = niter)
    ...
    for(iter in 1:niter) {
      ...
      if(time) {
        ...
        customSamplerTimes[["samplerTimes"]][[iter]] <<- samplerTimes#[1:(length(samplerTimes)-1)]
      } else {
        for(i in seq_along(samplerExecutionOrderToUse)) {
          ind <- samplerExecutionOrderToUse[i]
  ...
  methods = list(
    ...
    getCustomTimes = function() {
      returnType(double(2))
      return(as.matrix(customSamplerTimes, 'samplerTimes'))
    },
